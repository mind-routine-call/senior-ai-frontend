...
export default function MyPage() {
  ...
  if (loading) {
    return (
      <div className="flex h-full items-center justify-center bg-white text-lg font-bold text-gray-500">
        로딩 중입니다
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-full flex-col bg-white py-6 font-sans text-gray-900">
      <header className="mb-5 flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-bold text-[#FF6E61]">{isGuardian ? "보호자 마이페이지" : "어르신 마이페이지"}</p>
          <h1 className="mt-0.5 text-[22px] font-semibold leading-tight">
        ...
        {isGuardian ? (
          <>
                  ...
                  {elderList.map((elder) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-lg font-semibold">{elder.name || "이름 미입력"}</p>
            ...
            <section className="rounded-2xl bg-[#f6f6f6] p-4">
              ...
              >
                <span className="break-all text-xl font-semibold ">{inviteCode || "발급된 코드 없음"}</span>
                <button
                  type="button"
                  onClick={handleCopyCode}
                  disabled={!inviteCode}
          ...
          <>
            <section className="rounded-2xl bg-[#f6f6f6] p-4">
              <h2 className="mb-3 text-lg font-semibold">연결된 보호자</h2>
              <div className="flex items-center gap-3 rounded-2xl bg-white p-4">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#FFF3EE] text-[#FF6E61]">
                  <UserRound size={24} strokeWidth={2.5} />
                </div>
                <div>
                  <p className="text-lg font-semibold">{userData?.guardian_name || "연결된 보호자 없음"}</p>
            ...
            <section className="rounded-2xl bg-[#f6f6f6] p-4">
              <div className="mb-4 flex items-center gap-2">
                <Volume2 size={22} className="text-[#FF6E61]" strokeWidth={2.5} />
            ...
            <section className="rounded-2xl bg-[#f6f6f6] p-4">
              <h2 className="mb-4 text-lg font-semibold">글자 크기</h2>
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/45 px-5">
      <div className="max-h-[760px] w-full max-w-sm overflow-y-auto rounded-[28px] bg-white p-5 shadow-2xl">
        <div className="mb-5 flex items-start justify-between gap-3">
          <div>
            <p className="text-sm font-bold text-[#FF6E61]">
              {isGuardian ? "보호자 정보" : "어르신 정보"}
            </p>
            <h2 className="mt-1 text-[22px] font-semibold text-gray-900">개인정보 변경</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
          ...
          {!isGuardian && (
            <label className="flex flex-col gap-2">
              ...
              <textarea
                ...
                className="h-28 resize-none rounded-2xl bg-gray-100 px-4 py-3 text-base font-semibold leading-6 text-gray-900 outline-none focus:ring-2 focus:ring-[#FFB3AA]"
              />
            </label>
          )}
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          <button
            ...
            onClick={onClose}
            disabled={saving}
            className="rounded-2xl bg-gray-100 py-4 text-base font-semibold text-gray-700 disabled:opacity-50"
          >
            취소
          </button>
          <button
            ...
            onClick={onSubmit}
            disabled={saving}
            className="rounded-2xl bg-[#FF6E61] py-4 text-base font-semibold text-white shadow-sm disabled:opacity-50"
          >
            {saving ? "저장 중" : "저장"}
          </button>
        </div>
  ...
  return (
    <label className="flex flex-col gap-2">
      ...
      <input
        ...
        className="rounded-2xl bg-gray-100 px-4 py-4 text-base font-semibold text-gray-900 outline-none focus:ring-2 focus:ring-[#FFB3AA]"
      />
    </label>
  );
}
  ...
  return (
    <button
      ...
      className={`flex w-full items-center justify-between rounded-2xl px-4 py-4 text-left transition active:scale-[0.99] ${
        danger ? "text-red-600 hover:bg-red-50" : "text-gray-900 hover:bg-white"
      }`}
    >
      <span className="flex items-center gap-3 text-lg font-semibold">
        <span className={danger ? "text-red-500" : "text-[#FF6E61]"}>{icon}</span>
        {label}
      </span>
  ...
  return (
    <label className="mb-4 block last:mb-0">
      <div className="mb-2 flex items-center justify-between">
        ...
        <span className="text-sm font-bold text-[#FF6E61]">{value}</span>
      </div>
      <input
        type="range"
        min="1"
        ...
        className="w-full accent-[#FF6E61]"
      />
    </label>
  );
}
